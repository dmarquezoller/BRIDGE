...
dep_heatmap_ui <- function(tbl_name) {
    shinydashboard::box(
        ...
        shiny::column(
            width = 7,
            shinycssloaders::withSpinner(
            imageOutput(paste0("ht_", tbl_name), width = "500px", height = "500px"),  # <-- imageOutput
        ...
        shiny::column(
            ...
            h1(),
            p("Enable clustering:"),
            shinyWidgets::switchInput(paste0("clustering_", tbl_name), value = TRUE, onLabel = "YES", offLabel = "NO", width = 'auto'),
            shiny::numericInput(paste0("num_clusters_", tbl_name), "Select Number of Clusters", min = 2, step = 1, value = 3),
            shiny::numericInput(paste0("heat_pcutoff_", tbl_name), "FDR Threshold (-log10):",
                                min = 0, max = 10, value = 0.05, step = 0.01),
            shiny::numericInput(paste0("heat_fccutoff_", tbl_name), "FC Threshold (|log2FC|):",
                                min = 0, max = 10, value = 1, step = 0.1),
            shinyWidgets::actionBttn(paste0("recompute_heatmap_", tbl_name),
                                    shiny::span("Compute Heatmap", style = "color: white;"),
        ...
        h5(),
        shinydashboard::box(
        title = "Selected Genes", width = 12, solidHeader = TRUE, status = "info",
