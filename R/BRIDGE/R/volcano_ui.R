...
VolcanoUI  <- function(id, tbl_name, contrasts) {   
                        ...
                        shinydashboard::box(title = "Settings", width = 5, solidHeader = T, status = "info",
                            shinyWidgets::virtualSelectInput(ns(paste0("comparison_volcano_", tbl_name)), "Select Comparison:", choices = NULL),
                            shiny::selectizeInput(ns(paste0("volcano_search_",tbl_name)), "Search for Gene:", choices = contrasts, selected = contrasts[1], multiple=T),
