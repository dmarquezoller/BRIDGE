#' @export
dep2_proteomics <- function(df, tbl_name, ecols) {
  unique_pg <- DEP2::make_unique(df, names = "Gene_Name", ids = "Protein_ID", delim = ";")
  ecols <- match(ecols, colnames(unique_pg))
  ...
  sig_pg  <- DEP2::add_rejections(diff_pg)
  return(sig_pg)
}

#' @export
dep2_phosphoproteomics <- function(df, tbl_name, ecols) {
  df <- df[!grepl("p0", df$pepG), ] # Remove the p0
  unique_phos <-DEP2::make_unique(df, names = "pepG", ids = "Protein_ID", delim = ";")
  ecols <- match(ecols, colnames(unique_phos))
  ...
  sig_phos <- DEP2::add_rejections(diff_phos)
  return(sig_phos)
}

#' @export
dep2_rnaseq <- function(df, tbl_name, ecols) {
  unique_dds <- df
  ecols <- match(ecols, colnames(unique_dds))
